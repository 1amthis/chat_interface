 ...
 *
 * Uses the OpenAI SDK with a custom base URL since Mistral's API
...
/**
 * Stream chat using Mistral AI Chat Completions API
 */
export async function* streamMistral(
  ...
  for (const msg of messages) {
    const content = toMistralContent(msg);
    if (msg.role === 'user') {
  ...
  for await (const chunk of stream) {
    ...
    if (finishReason === 'tool_calls') {
      ...
      for (const [, tc] of toolCalls) {
        try {
          ...
          console.warn(
            `[Mistral] Failed to parse tool call arguments for "${tc.name}":`,
            tc.args.slice(0, 200)
          );
        }
      }
    ...
    // Mistral may include usage in the final chunk
    if (chunk.usage) {
